      ...
      {gaps.length > 0 && (
        <div className="space-y-2.5">
          {gaps.map(gap => {
            ...
            return (
              <div key={gap.id} className="flex items-center gap-2 pl-3">
                <div className={cn("w-1 h-1 rounded-full shrink-0", level.color)} />
                <span className="text-xs text-muted-foreground flex-1 min-w-0 leading-tight">
                  {gap.name}
                </span>
          ...
          {competencies.length > 5 && (
            <button
              onClick={onOpenWorkspace}
              className="text-[10px] text-muted-foreground/60 hover:text-foreground transition-colors pl-3"
            >
