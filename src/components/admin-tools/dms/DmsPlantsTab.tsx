...
type SortCol = 'code' | 'plant_name' | 'location' | 'is_active' | null;
...
const DmsPlantsTab: React.FC = () => {
  ...
  const { data: plants = [], isLoading } = useQuery({
    queryKey: ['dms-plants'],
    queryFn: async () => {
      const { data, error } = await supabase.from('dms_plants').select('*').order('display_order', { ascending: true });
  ...
  const createPlant = useMutation({
    mutationFn: async (item: { code: string; plant_name: string; location: string; is_active: boolean }) => {
      ...
      const { error } = await supabase.from('dms_plants').insert({ ...item, display_order: maxOrder + 1 });
      if (error) throw error;
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['dms-plants'] }); toast.success('Plant created'); setSheetOpen(false); },
  ...
  const updatePlant = useMutation({
    mutationFn: async (item: { id: string; code: string; plant_name: string; location: string; is_active: boolean }) => {
      const { id, ...rest } = item;
      const { error } = await supabase.from('dms_plants').update({ ...rest, updated_at: new Date().toISOString() }).eq('id', id);
      if (error) throw error;
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['dms-plants'] }); toast.success('Plant updated'); setSheetOpen(false); },
    onError: (err: any) => toast.error(err.message || 'Failed to update plant'),
  });

  const deletePlant = useMutation({
    mutationFn: async (id: string) => { const { error } = await supabase.from('dms_plants').delete().eq('id', id); if (error) throw error; },
  ...
  }, [plants, searchQuery, sortCol, sortDir]);

  const openAddSheet = () => { setEditingItem(null); setFormCode(''); setFormPlantName(''); setFormLocation(''); setFormIsActive(true); setSheetOpen(true); };
  const openEditSheet = (item: PlantRow) => { setEditingItem(item); setFormCode(item.code); setFormPlantName(item.plant_name); setFormLocation(item.location || ''); setFormIsActive(item.is_active); setSheetOpen(true); };

  const handleSave = () => {
    if (!formCode.trim() || !formPlantName.trim()) { toast.error('Code and Plant name are required'); return; }
    const payload = { code: formCode.trim(), plant_name: formPlantName.trim(), location: formLocation.trim(), is_active: formIsActive };
    if (editingItem) { updatePlant.mutate({ id: editingItem.id, ...payload }); }
    else { createPlant.mutate(payload); }
  };
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between py-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" className="gap-1.5" onClick={openAddSheet}><Plus className="h-4 w-4" /> Add Plant</Button>
          </div>
        </CardHeader>
            ...
            <Table>
              <TableHeader>
                <TableRow className="hover:bg-transparent">
                  ...
                  <TableHead className="text-xs font-medium uppercase tracking-wider text-muted-foreground cursor-pointer select-none" onClick={() => toggleSort('plant_name')}>Plant<SortIcon col="plant_name" /></TableHead>
                  <TableHead className="w-28 text-xs font-medium uppercase tracking-wider text-muted-foreground cursor-pointer select-none" onClick={() => toggleSort('location')}>Location<SortIcon col="location" /></TableHead>
              ...
              <TableBody>
                {sorted.map((item, idx) => (
                  <TableRow key={item.id} className="group border-border/40 hover:bg-muted/30 transition-colors cursor-pointer" onClick={() => openEditSheet(item)}>
                    ...
                    <TableCell><span className="inline-flex items-center justify-center h-6 min-w-[2rem] px-1.5 rounded bg-muted text-xs font-mono font-medium text-muted-foreground">{item.location || '—'}</span></TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive" onClick={e => { e.stopPropagation(); deletePlant.mutate(item.id); }}><Trash2 className="h-3.5 w-3.5" /></Button>
                      </div>
                    </TableCell>
                  </TableRow>
                ))}
                {sorted.length === 0 && !isLoading && (<TableRow><TableCell colSpan={6} className="text-center py-8 text-muted-foreground">No plants found</TableCell></TableRow>)}
              </TableBody>
            </Table>
          )}
        </CardContent>
        ...
        <SheetContent side="right" className="sm:max-w-md z-[150] flex flex-col">
          ...
          <div className="space-y-4 py-4 flex-1 overflow-y-auto">
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                ...
                <Input value={formCode} onChange={e => setFormCode(e.target.value.toUpperCase())} placeholder="e.g. C001" maxLength={10} className="font-mono" />
              </div>
              <div className="space-y-1.5">
            ...
            <div className="space-y-1.5">
              <Label className="text-sm font-medium">Plant Name <span className="text-destructive">*</span></Label>
              <Input value={formPlantName} onChange={e => setFormPlantName(e.target.value)} placeholder="e.g. North Rumaila CS1" />
            </div>
            <div className="flex items-center justify-between rounded-lg border p-3">
              <div><Label className="text-sm font-medium">Active Status</Label><p className="text-xs text-muted-foreground mt-0.5">Enable or disable this plant</p></div>
          ...
          <SheetFooter className="pt-4 border-t gap-2">
            ...
            <Button onClick={handleSave} disabled={isSaving} className="min-w-[100px]">
              ...
              {editingItem ? 'Save Changes' : 'Create Plant'}
            </Button>
          </SheetFooter>
        </SheetContent>
      </Sheet>
