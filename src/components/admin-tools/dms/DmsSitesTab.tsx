...
const DmsSitesTab: React.FC = () => {
  ...
  const createSite = useMutation({
    mutationFn: async (item: { code: string; site_name: string; comment: string; is_active: boolean }) => {
      ...
      const { error } = await supabase
        .from('dms_sites')
        .insert({ code: item.code, site_name: item.site_name, comment: item.comment || null, is_active: item.is_active, display_order: maxOrder + 1 });
      if (error) throw error;
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['dms-sites'] }); toast.success('Site created'); },
  ...
  const updateSite = useMutation({
    mutationFn: async (item: { id: string; code: string; site_name: string; comment: string; is_active: boolean }) => {
      const { error } = await supabase
        .from('dms_sites')
        .update({ code: item.code, site_name: item.site_name, comment: item.comment || null, is_active: item.is_active, updated_at: new Date().toISOString() })
        .eq('id', item.id);
      if (error) throw error;
    },
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['dms-sites'] }); toast.success('Site updated'); },
    onError: (err: any) => toast.error(err.message || 'Failed to update site'),
  });

  const deleteSite = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from('dms_sites').delete().eq('id', id);
      if (error) throw error;
    },
  ...
  const openEditDialog = (item: SiteRow) => {
    setEditingItem(item);
    setFormCode(item.code);
    setFormSiteName(item.site_name);
  ...
  const handleSave = () => {
    if (!formCode.trim() || !formSiteName.trim()) {
      toast.error('Code and Site name are required');
      return;
    }
    const payload = { code: formCode.trim(), site_name: formSiteName.trim(), comment: formComment.trim(), is_active: formIsActive };
    if (editingItem) {
      updateSite.mutate({ id: editingItem.id, ...payload });
    } else {
    ...
    <>
                  ...
                  <TableRow key={item.id} className="group border-border/40 hover:bg-muted/30 transition-colors">
                    ...
                    <TableCell className="text-right">
                      <div className="flex items-center justify-end gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <Button variant="ghost" size="icon" className="h-7 w-7 text-muted-foreground hover:text-destructive" onClick={() => deleteSite.mutate(item.id)}>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>{editingItem ? 'Edit Site' : 'Add Site'}</DialogTitle>
            <DialogDescription>
              {editingItem ? 'Update the site details' : 'Create a new site entry'}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4">
            <div className="space-y-2">
              <Label>Code *</Label>
              <Input value={formCode} onChange={e => setFormCode(e.target.value.toUpperCase())} placeholder="e.g. BNGL" maxLength={10} />
            </div>
            <div className="space-y-2">
