            ...
            <CardContent className="p-6 flex flex-col flex-1 overflow-hidden">
              <div className="space-y-3 flex-shrink-0 mb-4">
                <div className="relative">
                  <Textarea value={userInput} onChange={e => setUserInput(e.target.value)} onKeyPress={handleKeyPress} placeholder="Ask a question or describe what you need..." className="min-h-[100px] resize-none border-border/40 pr-24" disabled={isLoadingAI} />
                  <div className="absolute bottom-3 right-3 flex gap-2">
                    <Button size="icon" variant="ghost" onClick={handleVoiceInput} disabled={!isSupported || isLoadingAI} className="h-8 w-8">
                    ...
                    <Button size="icon" onClick={handleSend} disabled={isLoadingAI || !userInput.trim()} className="rounded-full bg-gradient-to-br from-primary to-accent h-8 w-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {quickActions.map(action => <Button key={action.id} variant="outline" size="sm" className="text-xs border-border/40 hover:bg-accent/20" onClick={() => setUserInput(action.label)}>
                      {action.label}
                    </Button>)}
                </div>
              </div>
          ...
          <Card className="flex-1 border-border/40 shadow-lg">
            ...
            <CardContent className="p-4">
              <div className="grid grid-cols-3 gap-3 h-full">
                ...
                return <Card key={workspace.id} onClick={() => onNavigate(workspace.id)} className="cursor-pointer border-border/40 hover:border-primary/30 transition-all hover:shadow-lg hover:scale-105 group">
