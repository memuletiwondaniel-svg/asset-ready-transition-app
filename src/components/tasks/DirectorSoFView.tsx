    ...
    <div className="space-y-6">
      <div className="max-w-3xl mx-auto">
        ...
        {pendingItems.map((item, index) => {
          ...
          const pssrIdParts = item.pssr?.pssr_id?.replace('PSSR-', '').split('-') || ['DP354'];
          const projectCode = pssrIdParts[0] || 'DP354'; // First part is the project code
          // Display with hyphen for readability (DP-354), use raw for color (DP354)
          ...
          {lockedItems.length > 0 && (
            <>
              ...
              {lockedItems.map((item) => {
                // Extract project code from PSSR ID (e.g., "PSSR-DP300-001" -> "DP300")
                const pssrIdParts = item.pssr?.pssr_id?.replace('PSSR-', '').split('-') || ['DP354'];
                const projectCode = pssrIdParts[0] || 'DP354';
                const displayId = projectCode.match(/^([A-Z]+)(\d+)$/) 
                  ? `${projectCode.match(/^([A-Z]+)(\d+)$/)?.[1]}-${projectCode.match(/^([A-Z]+)(\d+)$/)?.[2]}`
                  : projectCode;
                const projectName = item.pssr?.project_name || 'HM Additional Compressors';
