...
const CreateChecklistForm: React.FC<CreateChecklistFormProps> = ({
  ...
  // Checklist reasons
  const checklistReasons = ['Start-up or Commissioning of a new Asset', 'Restart following modification to existing Hardware, Safeguarding or Operating Philosophy', 'Restart following a process safety incident', 'Restart following a Turn Around (TAR) Event or Major Maintenance Activity', 'Others'];
  ...
  const handleItemToggle = (itemId: string) => {
    setFormData(prev => ({
      ...prev,
      selected_items: prev.selected_items.includes(itemId) ? prev.selected_items.filter(id => id !== itemId) : [...prev.selected_items, itemId]
    }));
  };
  const handleNext = () => {
  ...
  const filteredItems = (categoryItems: DBChecklistItem[]) => {
    return categoryItems.filter(item => {
      const matchesSearch = searchQuery === '' || item.description.toLowerCase().includes(searchQuery.toLowerCase()) || item.supporting_evidence && item.supporting_evidence.toLowerCase().includes(searchQuery.toLowerCase()) || item.topic && item.topic.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  if (showCreateItem) {
    return <CreateChecklistItemForm onBack={() => setShowCreateItem(false)} onComplete={handleCreateNewItem} existingCategories={availableCategories} />;
  }

  // Show item success page
  if (showItemSuccess && newCreatedItem) {
    return <ChecklistItemSuccessPage newItem={newCreatedItem} onBackToChecklist={handleItemSuccessBack} onCreateAnother={handleCreateAnother} />;
  }

  // Show loading state
  if (isLoading) {
    return <div className="min-h-screen bg-gradient-to-br from-background via-muted/30 to-background flex items-center justify-center">
          ...
          <Card className="border border-border/20 bg-card/90 backdrop-blur-sm">
            ...
            <CardContent className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <Input id="name" placeholder="Enter a descriptive name for your checklist" value={formData.name} onChange={e => setFormData(prev => ({
                ...prev,
                name: e.target.value
              }))} className="h-12" />
              ...
              <div className="space-y-2">
                ...
                reason: value
                  ...
                  <SelectContent className="bg-popover border border-border shadow-lg z-50">
                    {checklistReasons.map(reason => <SelectItem key={reason} value={reason} className="cursor-pointer">
                        {reason}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
                ...
                {/* Custom Reason Input */}
                {formData.reason === 'Others' && <div className="mt-4 space-y-2">
                    <Label htmlFor="customReason" className="text-sm font-medium">
                      Please specify the reason
                    </Label>
                    <Input id="customReason" placeholder="Enter your custom reason" value={customReason} onChange={e => setCustomReason(e.target.value)} className="h-10" />
                  </div>}
              </div>

              {/* Action Buttons */}
              <div className="flex justify-end pt-6 border-t border-border/20">
                <Button onClick={handleNext} disabled={!formData.name || !formData.reason || formData.reason === 'Others' && !customReason} className="fluent-button bg-primary hover:bg-primary-hover px-8">
                  Next: Select Items
  ...
  return <div className="min-h-screen bg-gradient-to-br from-background via-muted/30 to-background">
        ...
        <div className="bg-card/50 backdrop-blur-sm border border-border/50 rounded-2xl p-6">
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            <div className="flex-1 relative max-w-md">
              ...
              <Input placeholder="Search checklist items..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
            </div>
            <div className="flex items-center space-x-4 text-sm">
              ...
              {customChecklistItems.length > 0 && <Badge variant="default" className="px-3 py-1 bg-green-100 text-green-700 border-green-200">
      ...
      <div className="max-w-7xl mx-auto px-8 mb-6">
        <div className="flex justify-between items-center">
          <Button variant="outline" onClick={() => setCurrentStep(1)} className="fluent-button hover:bg-secondary/80 hover:border-primary/20">
            <ArrowLeft className="h-4 w-4 mr-2" />
            Back to Step 1
          </Button>
          <div className="flex space-x-3">
            <Button variant="outline" onClick={() => setShowCreateItem(true)} className="fluent-button bg-green-50 hover:bg-green-100 border-green-200 text-green-700">
              <Plus className="h-4 w-4 mr-2" />
              Add new checklist item
            </Button>
            <Button onClick={handleComplete} className="fluent-button bg-primary hover:bg-primary-hover">
      ...
      <div className="max-w-7xl mx-auto px-8 pb-8">
        <Tabs value={selectedCategory} onValueChange={setSelectedCategory} className="space-y-6">
          <TabsList className="grid grid-cols-4 lg:grid-cols-7 h-auto p-1 bg-card/50 backdrop-blur-sm">
            ...
            return <TabsTrigger key={category.id} value={category.id} className="text-xs p-3 space-y-1">
              ...
              <CardContent>
                <ScrollArea className="h-96">
                  <div className="space-y-4">
                    {allChecklistItems.filter(item => searchQuery === '' || item.description.toLowerCase().includes(searchQuery.toLowerCase()) || item.supporting_evidence && item.supporting_evidence.toLowerCase().includes(searchQuery.toLowerCase())).map(item => <div key={item.id} className={`flex items-start space-x-4 p-4 border rounded-lg hover:bg-muted/20 transition-colors ${customChecklistItems.some(custom => custom.id === item.id) ? 'bg-green-50 border-green-200' : ''}`}>
                        <Checkbox checked={formData.selected_items.includes(item.id)} onCheckedChange={() => handleItemToggle(item.id)} className="mt-1" />
                        <div className="flex-1 space-y-2">
                          <div className="flex items-start justify-between">
                            <div className="flex items-center space-x-2">
                              <h4 className="font-medium text-sm">{item.id}</h4>
                              {customChecklistItems.some(custom => custom.id === item.id) && <Badge variant="default" className="text-xs bg-green-100 text-green-700 border-green-200">
                                  New
                                </Badge>}
                            </div>
          ...
          return <TabsContent key={category.id} value={category.id} className="space-y-4">
                <Card className="border border-border/20 bg-card/90 backdrop-blur-sm">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                  ...
                  <CardContent>
                    <ScrollArea className="h-96">
                      <div className="space-y-4">
                        {items.map(item => <div key={item.id} className={`flex items-start space-x-4 p-4 border rounded-lg hover:bg-muted/20 transition-colors ${customChecklistItems.some(custom => custom.id === item.id) ? 'bg-green-50 border-green-200' : ''}`}>
                            <Checkbox checked={formData.selected_items.includes(item.id)} onCheckedChange={() => handleItemToggle(item.id)} className="mt-1" />
                            <div className="flex-1 space-y-2">
                              <div className="flex items-center space-x-2">
                                <h4 className="font-medium text-sm">{item.id}</h4>
          ...
          <TabsContent value="not-selected" className="space-y-4">
            <Card className="border border-border/20 bg-card/90 backdrop-blur-sm">
              ...
              <CardContent>
                <ScrollArea className="h-96">
                  <div className="space-y-4">
                    {unselectedItems.map(item => <div key={item.id} className="flex items-start space-x-4 p-4 border rounded-lg bg-muted/10 opacity-60">
                        <Checkbox checked={false} onCheckedChange={() => handleItemToggle(item.id)} className="mt-1" />
                        <div className="flex-1 space-y-2">
                          <div className="flex items-start justify-between">
                            <h4 className="font-medium text-sm">{item.id}</h4>
                            <Badge variant="outline" className="text-xs">
                    ...
                    {unselectedItems.length === 0 && <div className="text-center py-8">
        ...
        {formData.selected_items.length > 0 && <div className="fixed bottom-8 right-8 z-50 animate-scale-in">
            <Card className="border-0 shadow-2xl bg-white/95 backdrop-blur-xl rounded-2xl w-80 overflow-hidden">
              ...
              <CardContent className="p-4 max-h-60 overflow-y-auto">
                ...
                if (!item) return null;
                return <div key={item.id} className="flex items-center justify-between p-2 bg-gray-50 rounded-lg hover:bg-blue-50 transition-colors group">
                        <div className="flex items-center space-x-2">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => handleItemToggle(item.id)} className="opacity-0 group-hover:opacity-100 transition-opacity h-6 w-6 p-0 hover:bg-red-100">
                          <X className="w-3 h-3 text-red-500" />
                        </Button>
                      </div>;
              })}
                  {formData.selected_items.length > 5 && <div className="text-center py-2 text-xs text-gray-500">
