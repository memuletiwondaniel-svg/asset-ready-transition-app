...
const CreatePSSRWizard: React.FC<CreatePSSRWizardProps> = ({ open, onOpenChange, onSuccess, draftPssrId, mode = 'create' }) => {
  ...
  const generatePSSRId = async (): Promise<string> => {
    ...
    // Always use plant name as the code — station/field are sub-locations, not part of the PSSR ID
    if (plantName === 'UQ' && selectedField) {
      // UQ uses field code (e.g., "UQST - UQ Storage Terminal" -> "UQST")
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className={cn("h-full sm:h-[90vh] max-h-full sm:max-h-[90vh] overflow-hidden flex flex-col", currentStep === 5 && mode === 'lead-review' ? "sm:max-w-6xl" : "sm:max-w-3xl")}>
