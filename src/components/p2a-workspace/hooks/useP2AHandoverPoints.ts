...
export const useP2AHandoverPoints = (handoverPlanId: string) => {
  ...
  const { data: handoverPoints, isLoading } = useQuery({
    ...
    queryFn: async () => {
      // First get all phases for this plan
      ...
      // Get all handover points for these phases
  ...
  const createHandoverPoint = useMutation({
    mutationFn: async (data: {
      ...
      // Get current max position_y for this phase
  ...
  const moveHandoverPointToPhase = useMutation({
    mutationFn: async ({ handoverPointId, newPhaseId }: { handoverPointId: string; newPhaseId: string }) => {
      const { data, error } = await supabase
        .from('p2a_handover_points')
