...
-- Strengthen dedup in auto_create_p2a_approval_task to also check approver_role
...
-- Also clean up any existing duplicates in user_tasks right now
-- Keep only the most recent task per user+plan+role combination
DELETE FROM public.user_tasks
WHERE id IN (
  SELECT id FROM (
    SELECT id,
      ROW_NUMBER() OVER (
        PARTITION BY user_id, metadata->>'plan_id', metadata->>'approver_role'
