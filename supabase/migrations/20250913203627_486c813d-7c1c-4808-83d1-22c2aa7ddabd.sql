...
-- Use quotes around "position" since it's a reserved keyword
...
RETURNS TABLE(
  ...
  "position" text  -- Add position field to the return type (quoted because it's a reserved keyword)
)
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT 
    ...
    p."position"  -- Include position field in the SELECT (quoted because it's a reserved keyword)
