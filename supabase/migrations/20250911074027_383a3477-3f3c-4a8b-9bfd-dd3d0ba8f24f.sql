DROP FUNCTION public.get_enhanced_user_management_data();

CREATE OR REPLACE FUNCTION public.get_enhanced_user_management_data()
 RETURNS TABLE(user_id uuid, email text, full_name text, first_name text, last_name text, company user_company, employee_id text, job_title text, department text, phone_number text, account_status text, status user_status, last_login_at timestamp with time zone, created_at timestamp with time zone, sso_enabled boolean, two_factor_enabled boolean, avatar_url text, role text, roles text[], projects text[], manager_name text, pending_actions integer, login_attempts integer, locked_until timestamp with time zone, password_change_required boolean, last_activity timestamp with time zone, ta2_discipline ta2_discipline, ta2_commission ta2_commission, functional_email_address text)
 LANGUAGE plpgsql
...
BEGIN
  ...
  GROUP BY 
    ...
    manager.full_name, us.last_activity, p.ta2_discipline, p.ta2_commission, p.functional_email_address;
END;
$function$