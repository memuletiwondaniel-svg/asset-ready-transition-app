...
BEGIN
  ...
  IF NOT v_is_adhoc_revert THEN
    ...
    RAISE NOTICE 'STATUS-UPDATE rowcount=% target=% task=%', v_diag_rc, v_new_task_status, p_task_id;
    RAISE NOTICE 'STATUS-UPDATE readback=%', (SELECT status FROM user_tasks WHERE id=p_task_id);
  ...
  IF v_is_p2a_revert THEN
    ...
    IF v_project_id IS NOT NULL THEN
      ...
      IF v_p2a_plan_id IS NOT NULL AND v_p2a_plan_status IN ('ACTIVE','COMPLETED','APPROVED') THEN
        ...
        RAISE NOTICE 'END-OF-CASCADE readback status=%', (SELECT status FROM user_tasks WHERE id=p_task_id);
      END IF;
    END IF;
  END IF;
