...
BEGIN
  ...
  IF v_baseline_id IS NOT NULL THEN
    ...
    ELSE
      ...
      v_scope_changed := (v_baseline_legacy_hash <> v_live_legacy_hash);
    END IF;
  ...
  UPDATE public.p2a_handover_points
     SET status = 'READY'::p2a_handover_point_status,
         ...
         execution_plan_submitted_by = CASE WHEN v_capture_submitter AND v_caller IS NOT NULL THEN v_caller ELSE execution_plan_submitted_by END,
         execution_plan_submitted_at = CASE WHEN v_capture_submitter AND v_caller IS NOT NULL THEN now() ELSE execution_plan_submitted_at END,
         updated_at = now()
