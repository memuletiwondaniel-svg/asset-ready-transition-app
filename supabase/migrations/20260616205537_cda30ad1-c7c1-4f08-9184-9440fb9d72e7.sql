...
BEGIN
  ...
  -- Derive current column from current status (mirrors client mapping)
  ...
  ---------------------------------------------------------------------------
  -- STANDARD MOVE (skip user_tasks/ora_plan_activities for ad-hoc revert —
  -- task_reviewers trigger handles those)
  ---------------------------------------------------------------------------
  ...
  IF v_is_p2a_revert THEN
    ...
    IF v_project_id IS NOT NULL THEN
      ...
      IF v_p2a_plan_id IS NOT NULL AND v_p2a_plan_status IN ('ACTIVE','COMPLETED','APPROVED') THEN
        ...
        -- Archive decided approvers (only if any)
        ...
        -- Reverted-to-Draft audit entry (cycle = max-after-archive, fallback 1)
        ...
        -- Patch ora_plan_activities P2A-01 row
        IF v_ora_activity_id IS NOT NULL THEN
          UPDATE ora_plan_activities
        ...
        -- Delete approver review tasks
        DELETE FROM user_tasks
         WHERE type = 'p2a_approval'
           AND metadata->>'plan_id' = v_p2a_plan_id::text;

        -- Delete VCR Gantt activities + linked VCR delivery plan tasks
        ...
        -- Delete ORI p2a_approval snapshot
        DELETE FROM ori_scores
         WHERE project_id = v_project_id
           AND snapshot_type = 'p2a_approval';

        -- Delete p2a_plan_approved notifications
  ...
  ---------------------------------------------------------------------------
  -- GENERIC REVERT — reuse existing reopen_task
  ---------------------------------------------------------------------------
  ...
  RETURN jsonb_build_object(
    ...
    'cascade', CASE
      WHEN v_is_p2a_revert THEN 'p2a_revert'
      WHEN v_is_ora_revert THEN 'ora_revert'
      WHEN v_is_adhoc_revert THEN 'adhoc_revert'
      WHEN v_is_generic_revert THEN 'generic_revert'
      ELSE 'standard'
    END
  );
END;
...
-- Allow authenticated callers to invoke the RPC (SECURITY DEFINER scopes privileges internally)
REVOKE ALL ON FUNCTION public.move_task_to_column(uuid, text, boolean, text, text, boolean) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.move_task_to_column(uuid, text, boolean, text, text, boolean) TO authenticated;
GRANT EXECUTE ON FUNCTION public.move_task_to_column(uuid, text, boolean, text, text, boolean) TO service_role;