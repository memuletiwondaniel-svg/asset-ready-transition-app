   ...
   FROM projects p
     ...
     LEFT JOIN LATERAL (
       SELECT (count(*))::integer AS team_count,
         (SELECT ptm2.user_id FROM project_team_members ptm2
           WHERE ptm2.project_id = p.id AND ptm2.is_lead = true LIMIT 1) AS team_lead_user_id
       FROM project_team_members WHERE project_team_members.project_id = p.id
     ) tm ON true
     LEFT JOIN profiles pr ON pr.user_id = tm.team_lead_user_id
     LEFT JOIN LATERAL (
       SELECT (count(*))::integer AS milestone_count,
         (count(*) FILTER (WHERE project_milestones.status = 'Completed'))::integer AS completed_milestone_count,
         (SELECT pm2.milestone_name FROM project_milestones pm2
           WHERE pm2.project_id = p.id AND pm2.status <> 'Completed'
           ORDER BY pm2.milestone_date LIMIT 1) AS next_milestone_name,
         (SELECT pm2.milestone_date FROM project_milestones pm2
           WHERE pm2.project_id = p.id AND pm2.status <> 'Completed'
           ORDER BY pm2.milestone_date LIMIT 1) AS next_milestone_date,
         bool_or(project_milestones.is_scorecard_project) AS is_scorecard
       FROM project_milestones WHERE project_milestones.project_id = p.id
     ) ms ON true
     LEFT JOIN LATERAL (
       SELECT (count(*))::integer AS document_count
       FROM project_documents WHERE project_documents.project_id = p.id
     ) dc ON true
  WHERE p.is_active = true AND COALESCE(p.lifecycle_status, 'active') = 'active';

GRANT SELECT ON public.projects_enriched TO anon, authenticated, service_role;
