-- Update function to sync complete profile including email and company
...
CREATE TRIGGER sync_profile_names_to_auth_trigger
  AFTER INSERT OR UPDATE OF first_name, last_name, full_name, avatar_url, phone_number, primary_phone, position, email, company ON public.profiles
  FOR EACH ROW
  EXECUTE FUNCTION public.sync_profile_names_to_auth();

-- Backfill all existing profiles with complete profile data
UPDATE auth.users u
