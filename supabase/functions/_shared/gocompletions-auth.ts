...
// parseRadAjaxDelta + parseRadGridTable bodies moved to gohub-parsers.ts
// (re-exported at the top of this file). Keep this section as a marker
// so call sites still find the documentation via grep.
//
// RadAjax delta format: `length|type|id|content|` segments. Concat the
// `updatePanel` segments; pass through non-delta payloads unchanged.
//
// RadGrid quirks the parser handles:
//   - Greedy </table> close via bracket-matching (nested rgCommandTable
//     / rgPager would otherwise truncate the master table).
//   - Headers come from standalone `<th class="rgHeader">` cells; fall
//     back to thead / `<tr rgHeader>` for older grid layouts.
//   - Data rows are rgRow/rgAltRow scoped to the master-table content
//     (avoids capturing rows from nested command tables).
//   - `&nbsp;` placeholder cells are passed through as `&nbsp;` and must
//     be filtered with `isEmptyPlaceholderCell` from `gohub-contract.ts`
//     at persist time, not silently dropped.
